...
#  - we consider forecasts generated 2 days after this saturday date
#  - week ahead targets are defined relative to this saturday date
first_forecast_week_end_date <- lubridate::ymd("2020-05-30")
...
scores_by_week <- all_scores %>%
  ...
  dplyr::group_by(
    model, forecast_week_end_date, base_target, location_type) %>%
  dplyr::summarize(
...
ggplot(
  data = scores_by_week %>%
    dplyr::filter(
      !grepl("transform_log", model, fixed = TRUE) &
      !grepl("model_ARIMA", model, fixed = TRUE)
    ),
  mapping = aes(x = forecast_week_end_date, y = wis, color = model, group = model)) +
  geom_point() +
  geom_line() +
  facet_wrap(base_target ~ location_type, scales = "free_y") +
  theme_bw()
...
scores_overall %>%
  filter(base_target == "wk ahead cum death", location_type == "US") %>%
  ungroup() %>%
  select(intercept, combine_method, missingness, quantile_groups,
    window_size, check_missingness_by_target, do_standard_checks,
    do_baseline_check, wis) %>%
  arrange(wis)


scores_overall %>%
  filter(base_target == "wk ahead cum death", location_type == "State") %>%
  ungroup() %>%
  select(intercept, combine_method, missingness, quantile_groups,
    window_size, check_missingness_by_target, do_standard_checks,
    do_baseline_check, wis) %>%
...
scores_overall %>%
  filter(base_target == "wk ahead inc death", location_type == "US") %>%
  ungroup() %>%
  select(intercept, combine_method, missingness, quantile_groups,
    window_size, check_missingness_by_target, do_standard_checks,
    do_baseline_check, wis) %>%
...
scores_overall %>%
  filter(base_target == "wk ahead inc death", location_type == "State") %>%
  ungroup() %>%
  select(intercept, combine_method, missingness, quantile_groups,
    window_size, check_missingness_by_target, do_standard_checks,
    do_baseline_check, wis) %>%
...
scores_overall %>%
  filter(base_target == "wk ahead inc case", location_type == "US") %>%
  ungroup() %>%
  select(intercept, combine_method, missingness, quantile_groups,
    window_size, check_missingness_by_target, do_standard_checks,
    do_baseline_check, wis) %>%
...
scores_overall %>%
  filter(base_target == "wk ahead inc case", location_type == "State") %>%
  ungroup() %>%
  select(intercept, combine_method, missingness, quantile_groups,
    window_size, check_missingness_by_target, do_standard_checks,
    do_baseline_check, wis) %>%
...
for(model_case_var in model_case_vars) {
  plot_path <- paste0(
    "code/application/retrospective-qra-comparison/ensemble-score-plots/",
    "wis-cum_inc_death-", model_case_var, "-by_week.pdf")
  pdf(plot_path, width = 12, height = 7)

  p <- ggplot(
    data = wis_by_week_and_location_type %>%
      dplyr::filter(location_type != "US") %>%
      dplyr::mutate(case_and_constraint = paste0(combine_method, get(model_case_var)))
    ) +
    geom_point(
    ...
    facet_grid(base_target ~ forecast_week_end_date, scales = "free_y") +
    theme_bw() +
...
train_set_wis <- purrr::map_dfr(
  ...
  function(result_filename) {
    ...
    if(model_case$missingness == "impute") {
      ...
      train_set_wis_one_model <- location_groups$qfm_train[[1]] %>%
        attr("row_index") %>%
        dplyr::mutate(
          train_wis = covidEnsembles::wis(
            y = location_groups$y_train[[1]],
            qfm = predict(
