...
//          local_quad = quad_dampening * local_quad + quad_sd * raw_quad[i - 2];
          ...
          local_trend = local_trend + local_quad + trend_sd * raw_trend[i - 2];
        }
...
//  real<lower=0> beta_df;
  ...
  vector[n_basis - 2] raw_trend;
  real<lower=0> quad_sd;
  vector[n_basis - 2] raw_quad;
...
//  raw_beta ~ student_t(beta_df, 0.0, 1.0);
  raw_beta ~ normal(0.0, 1.0);
  raw_trend ~ normal(0, 1);
  raw_quad ~ normal(0, 1);
//  raw_beta ~ normal(0, 1);
