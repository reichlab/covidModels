...
for (forecast_week_end_date in forecast_week_end_dates) {
  ...
  results_path <- paste0('weekly-submission/forecasts/COVIDhub-non_symmetrized_baseline/',
                         forecast_week_end_date + 2,
                         '-COVIDhub-non_symmetrized_baseline.csv')
  if (TRUE) {
#  if(!file.exists(results_path)) {
    ...
    for (measure in c('deaths', 'cases', 'hospitalizations')) {
      ...
      measure_results <- purrr::map_dfr(
        ...
        function(location) {
          ...
          baseline_fit <- covidModels::fit_quantile_baseline(
            tail(location_data$inc, 28 / days_per_time_unit),
            symmetrize = FALSE)
  ...
  covidModels::plot_forecasts_single_model(
    ...
    plots_root = 'weekly-submission/COVIDhub-non_symmetrized_baseline-plots/',
    forecast_date = forecast_week_end_date + 2,
    model_abbrs = "COVIDhub-non_symmetrized_baseline",
