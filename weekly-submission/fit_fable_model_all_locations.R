...
model_dir <- paste0('weekly-submission/', full_model_case, '/')
...
#      function(location) {
      foreach(location = unique(data$location), .combine = rbind) %dopar% {
        print(location)
        location_data <- data %>%
        ...
        if(all(location_data$cum == 0)) {
          ...
          first_inc <- data %>% group_by(location) %>% filter(cum > 0) %>% filter(cum == min(cum)) %>% pull(inc)
          ...
          for(h in seq_len(horizon)) {
            forecast_inc_trajectories[, h] <- temp$inc[[1]]$dist$x %>%
              temp$inc[[1]]$transform()
          }
